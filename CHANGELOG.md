...
- The hive: `.harness/hive` holds open items, controller-proven facts and agent claims for the project that owns the worker. `uh land`, `uh queue` and `uh verify` are the only writers of facts, each citing hashed controller evidence; facts, the intervention ledger and the land decision index are hash-chained, `uh hive verify` reports the first broken line, and land and queue refuse a broken chain. The guard stops any agent write of the hive and denies direct reads in favour of `uh hive show`, and only verified facts reach worker prompts, as an escaped data block. `uh hive import` seeds items from a markdown checklist.
...
- `uh land` read collected reviews from the checkout git's common directory belongs to, which is a different checkout when the project is itself a linked worktree, so every land in that layout was refused; it now reads them from the project that owns the worker worktrees.
- `uh ps` showed `turns=0` for Claude Code and ACP runs. It now takes the larger of the control file's and the run digest's count, and when neither has one counts turns from the run's events: one per distinct assistant message id for Claude Code, one per stretch of agent activity for ACP. Supervision counts Claude Code turns the same way, so `limits.max_turns` applies to Claude Code runs.
- Windows toasts were raised under an unregistered app id, which Windows drops without an error. The `windows-toast` preset now uses Windows PowerShell's registered AppUserModelID by default and takes `app_id` for another registered one; `uh notify test` reports a toast as a handoff rather than a delivery and says what every other success proved (exit status or HTTP status); `uh notify detect` reports toasts unavailable when they are turned off in Windows settings; every `uh notify` subcommand takes `--root`, and the handbook shows the different top-level shapes of the project and user notification files.
