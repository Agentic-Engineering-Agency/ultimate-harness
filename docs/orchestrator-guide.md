...
- What to do when it did not: Inspect `runtime.stderr.log` and `runtime-result.yaml` in the run directory. If capabilities mismatch, inspect `uh adapter capabilities` or pass `--force` to bypass capability matching.
