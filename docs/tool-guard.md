...
| `read_window` | Command Code only: a `read_file` with no `limit`, or a `limit` above 600 lines, on a file larger than 40,000 bytes, wherever the file lives. Command Code 1.62.1 exits with an uncaught `write EOF` after a single tool result of that size ([CommandCodeAI/command-code#859](https://github.com/CommandCodeAI/command-code/issues/859)); windowed reads of the same file work. Small files, windowed reads and files that cannot be stat'ed are allowed. Unlike the classes above, the reason carries no common suffix. | `CONTRACT: <target> is <bytes> bytes and too large to read whole. Read it in windows of at most 600 lines using offset and limit.` |
