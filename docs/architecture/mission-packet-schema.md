...
- `constraints` are **advisory** prompt directives for capable runtimes (enumerated in adapter prompts). They are **not** executed by `uh validate` or `uh verify`. Encode enforceable rules as `acceptance_criteria` with `check_command` or `verification.required_checks` with `command`. `uh validate` warns when a packet declares `constraints[]` but has no runnable checks (UH-130).
