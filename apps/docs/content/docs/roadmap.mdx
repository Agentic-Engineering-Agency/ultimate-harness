...
| hermes-proxy | active (verified against `hermes proxy start --provider nous`; manifest default `nousresearch/hermes-4-405b`) |
| openrouter | active (OpenAI-compat HTTP to openrouter.ai; `OPENROUTER_API_KEY`; live-smoke verified) |
| pi | active (vanilla pi agent CLI; shipped v0.7.0 #135/#150; live-smoke verified) |
...
- **Container sandbox backend** — registered as a fail-fast stub (v0.7.0, #137); needs a runtime wired (v0.8.0 spike + impl).
- **Honcho-memory follow-ups** — `codex`/`hermes` wired (v0.6.0); remaining (v0.9.0): expose `honcho_search`/`honcho_remember` as mission MCP tools + `runtime_config.honcho_memory` opt-out.
...
**Shipped (no longer proposals):** OpenRouter adapter; vanilla `pi` adapter; cross-runtime QA `uh mission run-all --runtimes` with side-by-side diff/sentinel comparison; sandbox backend abstraction + `directory` backend; verify-then-promote auto-trigger; Epic 2 TUI polish UH-48..53.

## Strategic (decisions needed)

- **Muta integration** — UH-as-consumer, UH-as-component, or independent? Needs a co-founder conversation. **Deferred entirely to post-1.0.**
