...
description: "Develop the Hermes dashboard plugin that surfaces Ultimate Harness through the public uh CLI and .harness/ artifacts."
---
...
The plugin `manifest.json` declares `version`, which drives the `plugin-v<version>` release tag and is bumped in lockstep with the harness release (e.g. `0.9.0`).
