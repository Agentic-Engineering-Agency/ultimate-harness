...
The backend interface stays the one declared in [`sandboxing.md`](./sandboxing) §"Backend interface". AgentFS implementations of each method:
...
- **Escape detection.** Any write attempt that resolves outside the mount MUST be reported as a security finding per [`sandboxing.md`](./sandboxing) §"Required safety rules". AgentFS exposes the namespace boundary cleanly, so the backend MUST log such attempts to `audit/events.ndjson` rather than swallowing them.
