...
[runtime adapter contract](./runtime-adapter-contract): mission packets in,
...
- Reuse of broker/gateway patterns (oh-my-pi `auth-broker`/`auth-gateway`) is parked as a future option — tracked in the [roadmap](../ROADMAP).
