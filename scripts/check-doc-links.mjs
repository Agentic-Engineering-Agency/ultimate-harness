...
//
// Scans markdown under `docs/`, extracts
...
// can resolve on disk.
//
// `docs/` is plain markdown whose links are file paths (foo.md). The docs
// site (apps/site) mirrors it at build time and rewrites the links, so this
// tree is the only one that needs checking. The mdxRouting branch below is
// kept for route-style trees.
//
