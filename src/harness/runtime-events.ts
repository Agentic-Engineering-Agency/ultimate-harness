...
function cancelledResult(
  ...
  return RuntimeResultSchema.parse({
    ...
    exit_code: 143,
    prompt_path: existing?.prompt_path ?? relativeArtifactPath(input.root, path.join(missionRunDir(input.root, input.missionId, runId), "prompt.md")),
    stdout_path: existing?.stdout_path ?? relativeArtifactPath(input.root, path.join(missionRunDir(input.root, input.missionId, runId), "runtime.stdout.log")),
    stderr_path: existing?.stderr_path ?? relativeArtifactPath(input.root, path.join(missionRunDir(input.root, input.missionId, runId), "runtime.stderr.log")),
    diff_path: existing?.diff_path ?? relativeArtifactPath(input.root, path.join(missionRunDir(input.root, input.missionId, runId), "diff.patch")),
    errors,
  });
}
