...
export async function planCodexRun(root: string, missionPath: string): Promise<CodexRunPlan> {
  ...
  const rawConfig = await loadRawCodexConfig(root);
  ...
  const sandboxMode = readOptionalStringConfig(rawConfig, "sandbox_mode", "workspace-write");
  const approvalPolicy = readOptionalStringConfig(rawConfig, "approval_policy", "never");
  validateOptionalBooleanConfig(rawConfig, "full_auto_compat");
...
async function loadRawCodexConfig(root: string): Promise<Record<string, unknown>> {
  const manifestPath = path.join(root, ".harness", "adapters", "codex.yaml");
