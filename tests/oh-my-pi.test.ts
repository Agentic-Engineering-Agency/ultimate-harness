...
describe("oh-my-pi output parsing", () => {
  ...
  test("streams UTF-8 child output before close and reports callback rejection", async () => {
    ...
    expect(rejected.spawnError).toContain("callback boom");
  });
...
describe("uh mission run --runtime oh-my-pi", () => {
  ...
  test("retains mixed-route spend without last-model attribution or masked invalid counters", async () => {
    const { missionDir, missionPath } = await writeHarnessMission("mixed-route-accounting");
    const events = [
      { type: "message_end", message: { role: "assistant", provider: "provider-a", model: "model-a", content: "first", usage: { input: -1, output: 2, totalTokens: 10, cost: { total: 0.05 } } } },
      { type: "message_end", message: { role: "assistant", provider: "provider-b", model: "model-b", content: "done", usage: { input: 11, output: 3, totalTokens: 15, cost: { total: 0.05 } } } },
    ];
    const result = await runOhMyPi(TEST_ROOT, missionPath, {
      runner: async () => ({ stdout: events.map(event => JSON.stringify(event)).join("\n"), stderr: "", exitCode: 0, timedOut: false }),
      collectDiff: async () => ({ patch: "" }),
    ...
    expect(persisted.usage.output_tokens).toBe(5);
    expect(persisted.usage.input_tokens).toBeUndefined();
    expect(persisted.provider).toBeUndefined();
    expect(persisted.model).toBeUndefined();
    expect(persisted.usage.provider).toBeUndefined();
    expect(persisted.usage.model).toBeUndefined();
    const restricted = await runOhMyPi(TEST_ROOT, missionPath, {
      extraRuntimeConfigOverrides: { model: "provider-a/model-a" },
      runner: async () => ({ stdout: events.map(event => JSON.stringify(event)).join("\n"), stderr: "", exitCode: 0, timedOut: false }),
      collectDiff: async () => ({ patch: "" }),
    });
    expect(restricted.exitCode).not.toBe(0);
    expect(restricted.result?.status).toBe("failed");
  });
  ...
  test("rejects reuse of an attempt without replacing its existing evidence or launching a child", async () => {
    ...
    await writeFile(eventPath, "prior attempt evidence\n");
    let childStarted = false;
    await expect(runOhMyPi(TEST_ROOT, missionPath, {
      runId,
      runner: async () => {
        childStarted = true;
        return { stdout: "", stderr: "", exitCode: 0, timedOut: false };
    ...
    expect(await readFile(eventPath, "utf8")).toBe("prior attempt evidence\n");
  });
...
test("guard policy artifact and explicit extension are added only for guarded missions", async () => {
  const { missionDir, missionPath } = await writeHarnessMission("guarded-omp");
  await writeFile(missionPath, `${await readFile(missionPath, "utf8")}guard:\n  write_roots:\n    - out\n`);
  let seenInput: { args: string[]; env?: NodeJS.ProcessEnv } | undefined;
  const stdout = JSON.stringify({ type: "run_end", result: { finalText: "Complete", stopReason: "end_turn" } });
  const result = await runOhMyPi(TEST_ROOT, missionPath, {
    runId: "guarded-run",
    runner: async input => {
      seenInput = { args: input.args, env: input.env };
      return { stdout, stderr: "", exitCode: 0, timedOut: false };
    },
    collectDiff: async () => ({ patch: "" }),
  });
...
test("deadline recovery prompt includes one mandatory delivery instruction", () => {
  const prompt = recoveryPrompt({
    sourceRunId: "deadline-source",
  ...
  expect(prompt.match(/Your time budget is exhausted\./g)).toHaveLength(1);
  expect(prompt).toContain('section titled "Missing for the next step"');
});
